...
class QnaModel extends Model
{
    ...
    protected $allowedFields = ['user_id','title','content','answer','answer_date','answer_id','del_yn','del_date','reg_id','reg_ip','reg_date','mod_id','mod_ip','mod_date'];
    ...
    public function getQnaList(array $arrayParams = [], bool $total_cnt = false , int $s_limit = 0 , int $e_limit = 50)
    {
        ...
        if(empty($arrayParams['isAnswer']) == false){
            if($arrayParams['isAnswer'] == 'Y') $whereQueryString .= " AND answer_date IS NOT NULL ";
            else $whereQueryString .= " AND answer_date IS NULL ";
        }
