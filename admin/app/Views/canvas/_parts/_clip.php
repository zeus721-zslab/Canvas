...
<div class="d-flex flex-column mt-4">
    <div class="d-flex position-relative">
        <input name="search-text" type="text" class="form-control" style="padding-left: 2.5rem;box-sizing: border-box" value="" placeholder="요소 검색" />
        <input name="search-text-hidden" type="hidden" value=""/>
        <input name="search-type" type="hidden" value="_clip"/>
...
<!-- list contents -->
<div class="d-flex flex-column mt-4 overflow-y-scroll overflow-x-hidden scroll-ajax contents-list-container" data-act="category" style="padding-right: .75rem">
                ...
                <div class="image-list" style="overflow: hidden;width: 100vw">
                    <div class="image-list-inner justified-gallery editor_imglistall">
                        <?php foreach ($r['clips'] as $kk => $rr) { ?>
                            <a class="<?=$kk == 0 ? "active" : ""?>">
                                <img src="<?=$rr['thumb_file']?>" data-org_file="<?=$rr['save_file']?>" class="objItemImg canvas-img" data-w="<?=$rr['img_w']?>" data-h="<?=$rr['img_h']?>" alt="" />
                            </a>
                        <?php }?>
