...
<div class="overflow-y-scroll overflow-x-hidden scroll-ajax upload-file-wrap" data-act="category">
    ...
    <?php } else { //<!--  업로드된 파일이 있는 경우  -->?>
        ...
        <div class="overflow-y-scroll overflow-x-hidden scroll-ajax" style="padding-right: 0.75rem;" data-act="category">
            <div class="justified-gallery upload-file-list contents-list-container">
                <?php foreach ($aMyImg as $r) {?>
                    <a>
                        <img src="<?=$r['image_file']?>" data-org_file="<?=$r['image_file']?>" data-w="<?=$r['w']?>" data-h="<?=$r['h']?>" class="objItemImg objItemImgMy canvas-img" />
                        <span class="myimg-delete" onclick="myupload_delete('<?=$r['myimg_id']?>');">
                            <img src="/images/canvas_myimg_del.png" alt="" />
                        </span>
                    </a>
                <?php } ?>
            </div>
...
<script>
    ...
    function print_my_img(data){
        ...
        //html += '<a> <img src="'+data.image_file+'" data-org_file="'+data.image_file+'" data-w="'+data.w+'" data-h="'+data.h+'" class="objItemImg objItemImgMy canvas-img" /> </a>';
        html += '<a>';
        html += '   <img src="'+data.image_file+'" data-org_file="'+data.image_file+'" data-w="'+data.w+'" data-h="'+data.h+'" class="objItemImg objItemImgMy canvas-img" /> ';
