...
<main class="app-main"> <!--begin::App Content Header-->
    <div class="app-content-header"> <!--begin::Container-->
        <div class="container-fluid"> <!--begin::Row-->
            <div class="row">
                <div class="col-sm-6">
                    <h3 class="mb-0">템플릿 관리</h3>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-end">
                        <li class="breadcrumb-item"><a href="/">Home</a></li>
                        <li class="breadcrumb-item active" aria-current="page"> 템플릿 관리 </li>
                    </ol>
                </div>
            </div> <!--end::Row-->
        </div> <!--end::Container-->
                    ...
                    <div class="row mb-3">
                        <div class="col-md-2 col-sm-12">
                            <div class="form-group">
                                <label style="margin-right: 20px;">형태</label>
                                <select class="form-control" name="rotate" style="width: calc(100% - 72px);display: inline-block;" title="">
                                    <option value="">전체</option>
                                    <option value="P">세로형</option>
                                    <option value="L">가로형</option>
                                    <option value="S">정사각형</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-2 col-sm-12">
                            <div class="form-group">
                                <label style="margin-right: 20px;">사용여부</label>
                                <select class="form-control" name="use_flag" style="width: calc(100% - 88px);display: inline-block;" title="">
                                    <option value="">전체</option>
                                    <option value="Y">사용함</option>
                                    <option value="N">사용안함</option>
                                    <option value="I">작성중</option>
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-8 col-sm-12">
                            <div class="form-group">
                                ...
                                <button type="button" class="btn btn-success insertTemplate">템플릿 등록</button>
                                <button type="button" class="btn btn-danger delTemplate">선택삭제</button>
                            </div>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    function upsertForm(id = null){
        $.ajax({
            url: "/TemplateManagement/upsertForm",
    ...
    //키워드 관련
    $(document).on('keyup' , '#upsertForm input[name="keyword_str"]', function (e){
        e.preventDefault();
        ...
        if (e.keyCode === 13){
            ...
            var overlap = false;
            $('.keywordList button').each(function(){
                if( $(this).data('val') == kVal ) overlap = true
            })
            if(overlap === true){
                alert('이미 등록된 키워드입니다.');
                $(this).val("").focus();
            ...
            var html = "<button type=\"button\" class=\"btn btn-info text-white fs-7 py-1 px-2 mt-2 \"  data-val=\""+kVal+"\">"+kVal+"</button>";
            $('.keywordList').append(html);
            $(this).val("");

            $('.keywordList-callout-wrap').addClass('d-flex').removeClass('d-none');

        }

    });
    $(document).on('click' , '#upsertForm .keywordList button', function (e){
        e.preventDefault();
    ...
    $(function(){
        ...
        $('.insertTemplate').on('click',function(){ //회원등록 함수
            upsertForm();
        });

        $('.delTemplate').on('click',function(){ //선택삭제

            if($('input[name="template_id"]:checked').length < 1){
    ...
    function getList(page){
        ...
        var rotate       = $('select[name="rotate"]').val();
        var use_flag       = $('select[name="use_flag"]').val();
    ...
    $(document).on('click','.copy-template',function(){
        ...
        if(confirm('선택하신 템플릿을 복사하시겠습니까?')){
            ...
            $.ajax({
                url: "/TemplateManagement/copyTemplate",
