...
function InitCanvas(nIdx,defData){
	...
	img_rotate.src = "/canvas/easycanvas/easycv_icon_rotate.png";
...
function addHistory(){
	...
	if(oCanvas.toDatalessJSON().objects.length > 0){//데이터가 있는 경우만 history 저장
		...
		if(!overlap){//해당 페이지가 첫 저장인 경우
			...
			aHistory[nHistory]["sCanvasData"] = aOriginal[nCurPageNo];
			nHistory++;
		}
	...
	if(curObj.length === 1){//obj가 1개만 클릭
		...
		if (curObj[0]){
			...
			if (curObj[0].type == 'textbox'){
				...
				// if (curObj[0].type == 'textbox'){ 최하단에 올것
...
function showModifiedError(data){
	...
	html +='            <button class="btn curr-copy">현재 작업으로 복사본 만들기</button>';
...
function setContentsWrap(t){
	...
	if( $contents_menu_wrap.hasClass('d-none') && $object_wrap.hasClass('d-flex')){
		$contents_menu_wrap.addClass('d-flex').removeClass('d-none');
		$object_wrap.removeClass('d-flex');
		return false;
	}
...
function printSearchResult(data, m , isAppend){
	...
	if(data.length < 1 && $('.result-list-wrap div a').length < 1){ //데이터가 없는 경우
		...
		$.each(data,function(k,r){
			...
			if( m === '_template'){
				html += '<a><img alt="" src="'+r.thumb_file+'" class="'+classType+' ui-draggable ui-draggable-handle canvas-img" data-idx="'+r.template_id+'" alt="'+r.title+'" /></a>';
			}else if(r.save_file.indexOf('pdf') <= -1){
				html += '<a><img alt="" src="'+r.thumb_file+'" data-org_file="'+r.save_file+'" class="'+classType+' canvas-img" data-w="'+r.img_w+'" data-h="'+r.img_h+'" alt="'+r.title+'" /></a>';
			}else{
				console.log('pdf');
			}
	...
	if(type === '_bg' || type === '_clip') {
		...
		$.each(data, function (k, r) {
			...
			$.each(r.clips, function (kk, rr) {
				...
				html += '            <a ' + a_class + '>';
				html += '                <img src="' + rr.thumb_file + '" data-org_file="' + rr.save_file + '" class="'+img_class+' canvas-img" data-w="' + rr.img_w + '" data-h="' + rr.img_h + '" alt="" />';
				html += '            </a>';
		...
		$.each(data, function(k , r) {
			...
			//html += '<a> <img src="' + r.image_file + '" data-org_file="' + r.image_file + '" data-w="' + r.w + '" data-h="' + r.h + '" class="objItemImg objItemImgMy canvas-img" /> </a>';
			html += '<a>';
			html += '	<img src="' + r.image_file + '"  data-mid="'+r.myimg_id+'" data-org_file="' + r.image_file + '" data-w="' + r.w + '" data-h="' + r.h + '" class="objItemImg objItemImgMy canvas-img" />';
		...
		$.each(data, function(k , r){
			...
			$.each(r.templates , function(kk, rr){
				...
				html += '            <a '+a_class+'>';
				html += '                <img src="'+rr.thumb_file+'" class="objItemTemplate canvas-img" data-idx="'+rr.template_id+'"  alt="" />';
				html += '            </a>';
